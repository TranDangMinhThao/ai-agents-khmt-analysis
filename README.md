...
*(Xem hướng dẫn thêm ảnh ở phần [Cập nhật ảnh demo](#cập-nhật-ảnh-demo) bên dưới)*
...
Để thêm ảnh demo thật vào README:
1. Chụp ảnh các trang dashboard, lưu vào thư mục `docs/` trong repo (tạo thư mục này nếu chưa có).
2. Đặt tên đúng `screenshot-trangchu.png` và `screenshot-matrans.png` (hoặc đổi lại đường dẫn ảnh ở phần Demo phía trên cho khớp).
3. `git add docs/ README.md`, `git commit -m "Them anh demo"`, `git push`.
